...
export function useAsyncOperation<TArgs extends any[], TResult, TError = Error>(
  ...
  const executeWithRetry = useCallback(
    async (args: TArgs, attempt: number = 1): Promise<TResult> => {
      ...
      try {
        ...
        if (finalTimeoutConfig.timeout > 0) {
          timeoutRef.current = setTimeout(() => {
            const timeoutError = new Error(
              `Operation timed out after ${finalTimeoutConfig.timeout}ms`
            );
        ...
        if (cancellationTokenRef.current?.isCancelled) {
          throw new Error(
            `Operation cancelled: ${
              cancellationTokenRef.current.reason || "unknown"
            }`
          );
        }
        ...
        if (shouldRetry && !cancellationTokenRef.current?.isCancelled) {
          ...
          if (cancellationTokenRef.current?.isCancelled) {
            throw new Error(
              `Operation cancelled during retry: ${cancellationTokenRef.current.reason}`
            );
          }
...
export function useParallelAsync<TResult>(
  ...
  const progress =
    operations.length > 0 ? (completed / operations.length) * 100 : 0;
...
export function useDebouncedAsync<TArgs extends any[], TResult>(
  ...
  const [debouncedExecute, setDebouncedExecute] =
    useState<(...args: TArgs) => Promise<TResult>>();
