...
export const useEnhancedStats = () => {
  ...
  const safeSetItem = useCallback((key, value) => {
    try {
      ...
      setError(
        new Error(
          "Errore nel salvataggio dei dati. Controlla lo spazio disponibile."
        )
      );
      return false;
    }
  ...
  const selectors = useMemo(() => {
    ...
    return {
      ...
      avgTimePerTest:
        testsCompleted > 0 ? Math.round(timeSpent / testsCompleted) : 0,
    };
  }, [
    stats,
    testHistory.length,
  ...
  const initializeData = useCallback(async () => {
    ...
    try {
      ...
      const historyData = safeGetItem(
        STORAGE_CONFIG.keys.testHistory,
        EMPTY_ARRAY
      );
      const performanceData = safeGetItem(
        STORAGE_CONFIG.keys.wordPerformance,
        INITIAL_WORD_PERFORMANCE
      );
    ...
    (wordId) => {
      ...
      if (!wordData || !wordData.attempts || wordData.attempts.length === 0) {
        return {
          ...
          attempts: [],
          recommendations: [
            "Parola mai testata - inizia con un test per vedere le performance",
          ],
        };
      }
  ...
  const getAllWordsPerformance = useCallback(() => {
    return Object.keys(wordPerformance)
      .map((wordId) => {
  ...
  const recordWordPerformance = useCallback(
    (word, isCorrect, usedHint, timeSpent) => {
  ...
  const calculateSmartTestDifficulty = useCallback(
    (testWords, getWordAnalysisFunc) => {
      const categories = {
        hard: [],
        medium: [],
        easy: [],
      ...
      const sizeAdjustment =
        totalWords > 50 ? -0.3 : totalWords < 15 ? +0.2 : 0;
      ...
      if (hardPercentage >= 50 || adjustedScore >= 1.5) {
        difficulty = "hard";
        difficultyReason = `Test difficile: ${hardPercentage.toFixed(
          1
        )}% parole problematiche (${hardCount}/${totalWords})`;
      } else if (easyPercentage >= 70 || adjustedScore <= -0.5) {
        difficulty = "easy";
        difficultyReason = `Test facile: ${easyPercentage.toFixed(
          1
        )}% parole consolidate/miglioranti (${easyCount}/${totalWords})`;
      } else {
        difficulty = "medium";
        difficultyReason = `Test bilanciato: ${hardPercentage.toFixed(
          1
        )}% difficili, ${easyPercentage.toFixed(
          1
        )}% facili (${totalWords} parole)`;
      }
      ...
      const difficultyAnalysis = {
        ...
        distribution: {
          hard: {
            count: hardCount,
            percentage: parseFloat(hardPercentage.toFixed(1)),
          },
          medium: {
            count: mediumCount,
          ...
          easy: {
            count: easyCount,
            percentage: parseFloat(easyPercentage.toFixed(1)),
          },
        },
        statusBreakdown: {
          critical: categories.hard.filter((item) => item.status === "critical")
            .length,
          inconsistent: categories.hard.filter(
            (item) => item.status === "inconsistent"
          ).length,
          struggling: categories.hard.filter(
            (item) => item.status === "struggling"
          ).length,
          promising: categories.medium.filter(
            (item) => item.status === "promising"
          ).length,
          new: categories.medium.filter((item) => item.status === "new").length,
          improving: categories.easy.filter(
            (item) => item.status === "improving"
          ).length,
          consolidated: categories.easy.filter(
            (item) => item.status === "consolidated"
          ).length,
        },
      };
  ...
  const handleTestComplete = useCallback(
    (testStats, testWordsUsed, wrongWordsArray) => {
      const usedChapters = [
        ...new Set(
          testWordsUsed.map((word) => word.chapter || "Senza Capitolo")
        ),
      ];

      const chapterStats = {};
      usedChapters.forEach((chapter) => {
        const chapterWords = testWordsUsed.filter(
          (word) => (word.chapter || "Senza Capitolo") === chapter
        );
        const chapterWrongWords = wrongWordsArray.filter(
          (word) => (word.chapter || "Senza Capitolo") === chapter
        );
        ...
        chapterStats[chapter] = {
          ...
          percentage:
            chapterWords.length > 0
              ? Math.round(
                  ((chapterWords.length - chapterWrongWords.length) /
                    chapterWords.length) *
                    100
                )
              : 0,
      ...
      const { difficulty, difficultyAnalysis } = calculateSmartTestDifficulty(
        testWordsUsed,
        getWordAnalysis
      );
      ...
      const testRecord = {
        ...
        percentage: Math.round(
          (testStats.correct / (testStats.correct + testStats.incorrect)) * 100
        ),
        ...
        difficultyAnalysis,
        legacyDifficulty:
          testWordsUsed.length < 10
            ? "easy"
            : testWordsUsed.length < 25
            ? "medium"
            : "hard",
      };
      ...
      newStats.timeSpent +=
        testStats.totalTime || Math.round(Math.random() * 10) + 5;
      ...
      console.log(
        `✅ Test completato! Risultato: ${testRecord.percentage}% (Difficoltà: ${difficulty})`
      );
      return testRecord;
    },
    [
  ...
  const optimizedMigration = useCallback(() => {
    ...
    const migrationData = testHistory.reduce(
      (acc, test) => {
        ...
        if (test.chapterStats) {
          Object.entries(test.chapterStats).forEach(
            ([chapter, chapterData]) => {
              ...
              acc.categoriesProgress[chapter].correct +=
                chapterData.correctWords || 0;
              acc.categoriesProgress[chapter].total +=
                chapterData.totalWords || 0;
              acc.categoriesProgress[chapter].hints +=
                chapterData.hintsUsed || 0;
            }
          );
        }
    ...
    const migratedStats = {
      ...
      averageScore:
        migrationData.correctAnswers + migrationData.incorrectAnswers > 0
          ? (migrationData.correctAnswers /
              (migrationData.correctAnswers + migrationData.incorrectAnswers)) *
            100
          : 0,
  ...
  const exportData = useCallback(() => {
    try {
      ...
      const exportData = {
        ...
        description:
          "Backup completo v2.4: parole + statistiche + cronologia test + performance parole + difficoltà intelligente",
      };
      ...
      link.href = url;
      link.download = `vocabulary-complete-backup-v2.4-${
      ...
      console.log(
        `✅ Backup v2.4 esportato! (${words.length} parole + ${
          testHistory.length
        } test + ${Object.keys(wordPerformance).length} performance)`
      );
    } catch (error) {
  ...
  const importData = useCallback(
    (jsonString) => {
      try {
        ...
        const hasWords =
          importedData.words && Array.isArray(importedData.words);
        const hasStats =
          importedData.stats && typeof importedData.stats === "object";
        const hasHistory =
          importedData.testHistory && Array.isArray(importedData.testHistory);
        const hasWordPerformance =
          importedData.wordPerformance &&
          typeof importedData.wordPerformance === "object";

        if (!hasWords && !hasStats && !hasHistory) {
          throw new Error(
            "File non contiene dati validi (parole, statistiche o cronologia)"
          );
        }

        const isNewFormat = importedData.version === "2.4" && hasWords;
        const isEnhancedBackup =
          importedData.version === "2.3" && hasWordPerformance;

        let confirmMessage = "";
        if (isNewFormat) {
          confirmMessage = `Backup Completo v2.4 rilevato (${
            importedData.words?.length || 0
          } parole + ${importedData.testHistory?.length || 0} test + ${
            Object.keys(importedData.wordPerformance || {}).length
          } performance).\nOK = Sostituisci tutto | Annulla = Combina`;
        } else if (isEnhancedBackup) {
          confirmMessage = `Backup Enhanced v2.3 rilevato (${
            importedData.testHistory?.length || 0
          } test + ${
            Object.keys(importedData.wordPerformance || {}).length
        ...
        if (shouldOverwrite) {
          ...
          if (hasWordPerformance)
            components.push(
              `${Object.keys(newWordPerformance).length} performance`
            );

          console.log(
            `✅ Backup ${
              isNewFormat ? "v2.4" : isEnhancedBackup ? "v2.3" : "standard"
            } importato! ${components.join(" + ")}`
          );
        } else {
          // Merge data
          if (hasHistory) {
            const existingIds = new Set(testHistory.map((test) => test.id));
            const newTests = importedData.testHistory.filter(
              (test) => !existingIds.has(test.id)
            );
            newHistory = [...testHistory, ...newTests].sort(
          ...
          if (hasWordPerformance) {
            ...
            Object.entries(importedData.wordPerformance).forEach(
              ([wordId, data]) => {
                if (newWordPerformance[wordId]) {
                  const existingAttempts =
                    newWordPerformance[wordId].attempts || [];
          ...
          if (hasWords) {
            ...
            const existingEnglish = new Set(
              currentWords.map((w) => w.english.toLowerCase())
            );
            const newWords = importedData.words.filter(
              (word) => !existingEnglish.has(word.english.toLowerCase())
            );
  ...
  useEffect(() => {
    ...
    const shouldMigrate =
      ...
      !optimizationState.isProcessing;
